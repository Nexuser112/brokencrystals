        ...
        stage('SAST') {
            parallel {
                stage('CodeQL') {
                    steps {
                        script {
                            sh 'bash -c "${USER_BIN}/codeql database create codeql-db --language=javascript --source-root=${SCAN_DIR}"'
                            sh 'bash -c "${USER_BIN}/codeql analyze codeql-db --format=sarifv2.1.0 --output=codeql-results.sarif"'
                        }
                    }
                }
